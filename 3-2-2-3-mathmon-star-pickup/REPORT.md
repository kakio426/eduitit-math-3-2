...
- 공개 입력 커밋: `3d72ae0eb0ded3f8b6a0a675863d724011639332`
- 입력 커밋 시각: `2026-09-07T21:13:27+09:00`
...
- 실행 진입점 SHA-256: `e861f5d07f5ef39ebc3c46de971ebde2ff4492266cf522e7630c552ee36c1610`
- Pages 파일 집합 SHA-256: `995b2e731411b44994905cd48cf97bd12cc7443cc0b0ad14ef3017326823b96a` (138개)
...
왼쪽 `300×656px` 슬롯에는 여섯 단계 유니콘 별자리판이 문제를 푸는 동안 계속 보입니다. 보상이 반영되면 별자리 안의 주요 별이 차례로 켜지고 연결선이 마지막에 한 번 더 밝아집니다. 보상 모달은 Stage 가운데에 열리고 별자리판은 뒤에서 흐리게 보입니다.

### 정답은 유리하지만 오답이 무조건 감점은 아님

모든 문제는 같은 랜덤 사건 확률표를 사용합니다. 처음에 맞히면 실제 나머지만큼 별빛을 더 받고, 불리한 사건이 나와도 현재 별빛을 지킵니다. 오답 뒤 다시 맞힌 문제도 `+`, `-`, `0`이 모두 나올 수 있으므로 일부러 운을 기대하며 다시 도전할 여지가 남습니다.
...
`별빛 보기`를 누르면 Stage 가운데 모달에서 바로 `별빛 점수 +N`, `별빛 점수 -N`, `별빛 점수 0` 중 하나가 보입니다. 별자리판은 뒤에서 흐려지고, 별빛이 반영되면서 별자리 안의 별이 차례로 켜집니다.
...
전체 흐름은 아래 여섯 화면 조건에서 각각 모든 상태를 다시 캡처했습니다.
...
- 깨진 이미지 0건
- 별자리판과 계산판 교차 0px
- 사용자 제보 viewport에서 Stage `1035.84375×647.3984375px`, 보상 카드 `430×480px`, Stage 중심 오차 `0.004px`
- 사용자 제보 viewport에서 보상 이미지 `250×250px`, 배경 블러 `10px`, 라벨·다음 버튼 교차 `0px`
- compact 3행 묶음 상태에서 식과 첫 별 묶음 사이 간격 `21.9px`
- 문제·보상·결과 상태의 텍스트 overflow 0건
- 결과 요소 형제 교차 0건
- 다시 버튼 불투명 사각 캔버스 0건
- 결과 이미지와 실제 클릭 영역 불일치 0건

Humanizer 기준으로 학생 문구도 다시 읽었습니다. `별빛 보기`, `별빛 점수 +N`, `별빛 점수 -N`, `별빛 점수 0`, `다음엔 ○○자리`, `모은 별빛 N`, `다시`는 짧고 행동이나 뜻이 분명합니다. 학생 화면에는 `핵심`, `등급`, `게이트`, `오브젝트` 같은 제작자 용어가 없습니다.

## 텍스트 넘침·요소 겹침 QA

- 확인 viewport: `1079×842`, DPR `1`, Stage `1035.84375×647.3984375px`
- 확인 상태: 첫 화면, 설명 1·2, 문제 대기, 몫 오답 양방향, 몫 정답 확인, 남은 별 대기·오답, 완성식, 열린 보상, 결과 6단계
- 식 위치: compact 3행 묶음에서 식 하단과 첫 묶음 상단 사이 `21.9px`; 식·별 묶음 교차 `0px`
- 보상 모달: 카드 `430×480px`, 이미지 `250×250px`, Stage 중심 오차 `x 0px / y 0.004px`; 라벨·버튼 교차 `0px`
- 자동 QA 결과: 텍스트 overflow `0건`, 요소 겹침 `0건`, 깨진 이미지 `0건`

## 11. 최신 자동 검증

2026-08-10 현재 코드에서 아래 검사를 다시 실행했습니다.
...
- 실행본 SHA-256: `4330afa1e2fc2ecc0988696c2a18fb3d6cfe939960d722fbb09599280d242818`
- 생성 시각: `2026-08-10T02:03:31.539Z`
- 등록 화면 크기: `6개`
