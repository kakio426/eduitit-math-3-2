...
- Stage-Reveal 보상 화면이 이 차시에만 있는 `center-art-bottom-route-v1` 띠 배치를 쓰고 있었습니다. 스킬이 정한 Stage-Reveal 표시 방식과 6단원 공용 구현에서 벗어난 배치였고, 정사각 그림이 Stage 가운데에만 놓여 좌우로 큰 빈 공간이 생기고 닫힌 상태에서는 변화량 칸과 진행 막대가 빈 상자로 보였습니다. 공용 `unit6-two-column-reveal-v1` 배치(왼쪽 사건 그림, 오른쪽 `현재 역 → 변화 → 다음 역 → 행동` 카드)로 되돌리고 열차 색만 차시 색으로 유지했습니다.
- 되돌린 뒤 보상 그림은 `1280×800`에서 닫힘 `481.3px`·열림 `510.6px`, `1024×768`에서 `438.0px`·`464.7px`, `1079×929`에서 `462.1px`·`490.5px`로 계약 범위 `180~512px` 안에 들어옵니다. 오른쪽 카드는 각각 `476.9×463.6px`, `400.8×383.3px`, `422.7×384.4px`입니다.
- `lesson.json`의 `qa.rewardStageAudit.layout`과 `qa.divisionTrainDesignFeedbackAudit.rewardLayout`, `scripts/test-unit2-three-digit-division.mjs`의 잠금도 `unit6-two-column-reveal-v1`으로 함께 바꿨습니다. 차시 전용 띠 배치가 다시 들어오면 테스트가 실패합니다.
...
- 실행본 SHA-256: `28f2ca303cf86808ad825882ffb7175a36028c9b2ed1a20ecba5599335fd80fd`
- 생성 시각: `2026-08-15T09:02:05.557Z`
- 등록 화면 크기: `5개`
