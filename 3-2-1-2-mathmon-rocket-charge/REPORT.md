...
순위 화면 문구는 Humanizer 기준으로 점검했습니다. 학생에게 보이는 말은 `순위 보기`, `전국 로켓 순위`, `내 이름`, `연료 점수`, `내 등수`, `10위까지`, `새로 보기`, `결과로`, `다시하기`처럼 짧게 유지했습니다. 개발자에게 필요한 말과 이름 정책 설명은 화면에 보이지 않습니다.
