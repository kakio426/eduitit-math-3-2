...
순위 화면은 결과 뒤에 붙는 마지막 화면입니다. `_shared/scoreboard` 공통 화면을 사용하며, `scoreboard-celebration-bg-generated.webp`는 매스몬, 불꽃, 무대 조명만 담은 축하 배경입니다. 순위판, 내 기록 3칸, 순위 행, 버튼 표면, 보이는 글자는 SVG `viewBox="0 0 1280 800"` 안에서 공통 컴포넌트가 그립니다. 더 이상 생성 이미지 안의 박스에 글자를 맞추지 않습니다. SVG `<text>`는 `전국 로켓 순위`, 이름, 연료 점수, 내 등수, 이번 주 순위 목록, 버튼 라벨처럼 실제 값을 그리고, HTML 버튼은 보이지 않는 hitbox만 맡습니다. 학생이 직접 이름을 쓰는 칸은 없습니다. 순위 기능이 꺼져 있으면 `선생님이 순위 기능을 켜면 전국 순위를 볼 수 있어요.`만 보여 주고 게임 결과는 그대로 유지합니다.
