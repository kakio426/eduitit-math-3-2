...
- 순위: 결과 뒤 `순위 보기`를 누르면 이번 주 전국 순위 화면으로 이동. API 주소가 설정된 경우에만 서버가 만든 기록 이름으로 점수를 제출하고 10위까지 보여 줌
...
- `08-scoreboard.png`: 전국 순위 화면
- `09-tablet-scoreboard.png`: 태블릿 가로 전국 순위 화면
...
- `../_shared/scoreboard/scoreboard-celebration-bg-generated.webp`: 모든 차시가 함께 쓰는 전국 순위 축하 배경
- `../_shared/scoreboard/scoreboard-title-rocket-generated.webp`: 2차시 전국 순위 생성형 타이틀 이미지

첫 화면과 결과 화면은 생성 이미지를 배경으로 씁니다. 첫 화면 제목은 GPT Image로 만든 별도 자산을 배경 위에 얹고, 시작 버튼은 1차시 포스터형 버튼 물성을 참고해 별도로 생성한 두툼한 노란 래스터 버튼 자산으로 보여 줍니다. 실제 클릭은 같은 위치의 HTML 버튼이 맡습니다. 첫 화면의 매스몬은 `cover-generated.webp` 장면 안에 포함되어 있고 별도 컷아웃 `<img>` 오버레이를 쓰지 않습니다. 한 줄 목표는 HTML로 보입니다. 결과 화면의 큰 `수성 도착!`부터 `안드로메다 도착!`, `다시 도전!` 문구는 `result-title-*-generated.webp` 생성형 타이틀 이미지가 맡고, 정답 수는 `_shared/result-count/result-correct-*-generated.webp` 생성형 숫자 이미지가 맡습니다. SVG 동적 레이어는 `연료 86` 같은 매 판 값과 버튼 표면만 그립니다. 결과 이미지는 텍스트 없이 로켓, 매스몬, 도착 장소만 담습니다. 문제 화면은 생성 이미지 발사장을 배경으로 깔고 기존 로켓 이미지를 얹습니다. 배터리 창 안쪽 액체 연료, 바닥 링, 증기, 신호등, 빛줄기 효과는 HTML/CSS 레이어로 얹어 연료량에 따라 반응하게 했습니다. 문제 계산은 왼쪽 세로셈판을 중심으로 보이게 하고, 오른쪽 보조식은 기본으로 숨긴 뒤 `힌트` 버튼을 눌렀을 때만 보여 줍니다. 연료 이벤트 모달은 6개 상황을 담은 스프라이트 이미지를 배경으로 쓰고, `연료 +6`, `연료 -12`, `연료 0`, `발사!`, `무지개!` 같은 짧은 값만 HTML로 얹습니다. 순위 화면은 `_shared/scoreboard` 공통 화면을 사용합니다. 생성 이미지는 매스몬과 불꽃이 있는 축하 배경과 상단 타이틀 이미지가 맡고, 순위판·내 기록 3칸·순위 행·버튼 표면·동적으로 바뀌는 글자는 SVG 컴포넌트가 직접 그립니다. HTML 버튼은 보이지 않는 hitbox만 맡습니다.

## 스코어보드 연동

기본 파일만 열면 순위 기능은 꺼진 상태로 동작합니다. 실제 서버를 붙일 때는 게임을 열기 전에 아래 값을 주입합니다.
...
게임은 시작할 때 세션을 만들고, 결과 뒤 `순위 보기`를 누른 순간 점수와 답안 로그를 제출합니다. API가 없거나 실패해도 게임 결과 화면은 그대로 유지됩니다.
...
- `result-title-*-generated.webp`: 도착지별 결과 생성형 타이틀 이미지
- `result-retry-generated.webp`: 다시하기 RasterStage 배경
- `eduitit-logo-mark.png`: 에듀잇티 로고
- `../_shared/scoreboard/*`: 공통 전국 순위 배경, 생성형 타이틀 이미지, CSS, 렌더러
