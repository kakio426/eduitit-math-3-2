...
마지막 바람 뒤 곧바로 결과를 고정하지 않고, `점프 길을 살펴봐요.` → `도착한 곳을 찾았어요.` → `섬이 보여요.` 순서로 짧게 보여 줍니다. 공개 뒤에는 도착지별 `result-final-*` 완성 이미지가 배경, 도착 라벨, 큰 결과 문구, 이미지 속 `다시하기` 버튼, 점수용 빈 네모 상자를 모두 담당합니다. 보이는 HTML/CSS 텍스트는 `6/10` 같은 정답 수 숫자 1개만 남겼습니다. `순위 보기`는 작은 생성형 버튼 자산으로 보이고, 순위와 다시하기 클릭은 투명 hitbox가 맡습니다. 점수 숫자는 스크린샷 픽셀에서 글자 중심과 빈칸 중심을 비교하는 QA로 확인합니다. 점수 상자의 `맞힌 문제` 라벨과 시작 결과의 보이는 `출발섬` 텍스트는 제거했습니다.
...
최종 결과는 도착 섬마다 다른 통이미지를 사용합니다. HTML로 보이는 것은 `10/10` 같은 정답 수 숫자뿐이고, 결과 라벨·다시하기 버튼 모양·점수용 빈칸은 이미지 안에 들어갑니다. `순위 보기`는 별도 생성형 버튼 이미지로 보이며, 버튼 동작은 투명 hitbox가 맡습니다.
