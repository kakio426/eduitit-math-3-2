...
- 제작 보고서 원본 SHA-256: `44a64d33dbe6445689083d02ba45e1583f6bc46bded02eb81c1c6ee35ac8c1bb`
- 실제 실행 진입점: `3-2-5-1-mathmon-water-fill/index.html`
- 실행 진입점 SHA-256: `9a0e8079d86a21859cd27781c1c9c5bcf2139702a958265b8ab0ab28859016d0`
- Pages 파일 집합 SHA-256: `659e68098b68e6da22025b7faff0511bc092504f857ede3bedfac633b20b0612` (140개)
...
## 2026-09-19 최종 화면·문항 교정

- 첫 화면 배경을 펭귄몬 전신과 제목 영역이 분리된 `cover-v2` 자산으로 교체하고 캐시 키를 갱신했습니다.
- 2~8번의 비교 통은 물이 통 밖으로 보이지 않도록 안쪽 안전 영역에 고정했고, `가 물통`은 하늘색, `나 물통`은 연보라색으로 문제·완성 문장까지 일관되게 구분했습니다.
- 질문과 지시 문구를 `크기와 모양이 같은 통`, `같은 종이컵`, `물을 옮겨 붓지 않고`처럼 3학년 학생이 바로 이해할 수 있는 말로 다듬고, 선택지와 완성 문장의 어색한 줄바꿈을 줄였습니다.
- 7번은 종이컵 표식을 더 크게 표시해 두 물통의 컵 수 차이를 눈으로 셀 수 있게 했습니다.
- 9번은 생성 이미지 속 실제 물통 중심 아래에 이름표를 맞췄습니다. 10번은 9번과 같은 두 물통이 이어지는 전용 장면으로 바꾸고, 같은 크기의 종이컵 그림과 `5개`·`7개` 표식을 각 물통 바깥쪽에 배치했습니다.
- 설명 화면의 `이전`·`문제 시작` 버튼을 아래 가장자리에서 위로 올렸습니다.
- 결과 화면은 배경 액자 위를 가리던 별도 사각 패널을 숨겼습니다. 단계 이름 이미지를 키우고 공용 `다시` 버튼의 비율과 클릭 영역을 맞췄습니다.
- 변경 실행본은 JavaScript 구문 검사와 Stage 비율 검사를 통과했고, 10번 문제의 종이컵·물통·이름표 배치를 사용자 제보 화면 크기에서 확인했습니다.
...
- 내장 공통 파일: `19개`
- 매니페스트: `assets/PORTABLE_RUNTIME_MANIFEST.json` (SHA-256 `6fe3611de8eda8fc8143bf7c26829e28d936f08f985dec2f7af9a6352bbdcb4c`)
...
## 2026-09-16 물통 비교 화면 교정

- 결과 화면의 `다시` 버튼을 공용 `mathmon-result-retry-button-v2`의 원본 비율에 맞춘 `190×95px` 슬롯으로 교정했습니다. 이미지와 클릭 영역의 네 변은 `1px` 이내로 일치해야 합니다.
- `다음엔 …` 문구를 짙은 남색 글자와 밝은 전용 라벨 표면으로 묶었습니다. 전용 회귀는 글자와 라벨 표면의 대비 `4.5:1` 이상을 검사합니다.
...
- title: `매스몬 물통 채우기 시합`
- learningGoal: 들이를 직접 비교하고 같은 종이컵으로 재어 비교하기
...
- 당시 Humanizer 학생 문구 QA에서는 `지금의 물통` 상태 라벨을 사용했습니다. 2026-09-16 교정 뒤에는 이 문구를 없애고 현재 단계 이름만 크게 보여 줍니다.
...
## 2026-09-18 최신 핵심 화면 스크린샷

- 실행본 SHA-256: `2fa9143875e3f2acb65a46e6e5aabdc558208175c622e9423b3c4fa884a86c35`
- 생성 시각: `2026-09-18T02:16:48.684Z`
- 등록 회귀 이름: `8개`
- 실제 실행 화면 조건: `7개`
- 동일 조건 별칭 통합: `1개`
