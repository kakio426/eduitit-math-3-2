...
- 랜덤 보상: 닫힌 표적 1장과 사건별 그림 6장으로 된 512×512 Stage-Reveal 이미지 세트
...
중심 보상은 표적 점수 하나입니다. `점수 보기`를 누르면 결과가 가려진 표적이 먼저 나오고, `가림막 열기`를 눌러야 증가·작은 손해·큰 증가·정중앙·0·무지개 중 하나가 공개됩니다. 오른쪽에는 `이번 점수`와 실제 변화량을 크게 두고, 아래에는 현재 표적과 다음까지 남은 값만 한 줄로 보여 줍니다. `0`은 누적 점수를 지우지 않으며, 손해는 흔한 정답의 최소 증가량보다 크지 않습니다. 최종 결과 그림은 이 화면에서 쓰지 않습니다.
