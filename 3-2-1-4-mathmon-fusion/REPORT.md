...
- 결과 등급: 최종 합체 점수만으로 소형 -> 중형 -> 대형 -> 거대 -> 전설 합체 중 하나를 공개
- 점수 기준: 소형 100점, 중형 300점, 대형 500점, 거대 1000점, 전설 1500점
...
결과 화면은 도달 등급별 RasterStage 배경과 다시하기 배경을 사용합니다. 현재 점수표의 runtime 등급은 소형, 중형, 대형, 거대, 전설입니다. 큰 등급 문구는 `result-title-*-generated.webp` 생성형 타이틀 이미지가 맡고, 정답 수는 `_shared/result-count/result-correct-*-generated.webp` 생성형 숫자 이미지가 맡습니다. SVG 동적 레이어는 실제 합체 점수, `순위 보기`, `다시하기` 버튼 표면을 정확한 좌표에 그립니다. 실제 클릭은 투명 HTML hitbox가 맡습니다. 점수 바와 버튼 사이에는 충분한 여백을 두어 붙어 보이지 않게 했습니다.
...
합체 점수가 100점보다 낮으면 `result-retry-generated.webp`를 RasterStage 배경으로 사용합니다. 로봇 부품이 안전하게 쉬고 있는 장면 위에 다시 도전 문구와 `다시하기` 버튼을 보여 줍니다.
...
- `result-small-generated.webp`부터 `result-legend-generated.webp`까지 결과 등급 이미지를 image generation으로 다시 만들었습니다. 현재 runtime 점수표는 소형, 중형, 대형, 거대, 전설을 사용합니다. 이전 초거대 이미지는 보존 자산으로만 남깁니다.
