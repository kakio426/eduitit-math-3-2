...
- 보상: 한 문제 완료마다 합체 힘 이벤트를 1회 적용. 정답 문제는 힘 충전, 강화 부품, 부품 결함, 완성 신호, 합체 실패, 무지개 코어 중 하나가 나오고, 오답이 있었던 문제는 부품 결함 처리만 적용. 어떤 보상이 나와도 10문제를 모두 푼 뒤에만 결과로 이동
...
합체 힘은 단일 중심 보상입니다. 한 판이 끝난 뒤 합체 힘 점수를 재고, 정해진 점수 기준에 맞춰 완성 로봇 등급을 공개합니다. 결과는 차시 자체 완결형 보상이며, 2차시의 행성 도달 구조와 같은 방식으로 `도달 등급`을 보여 줍니다.

## 3. 게임 흐름

```text
첫 화면 -> 설명 화면 -> 부품 1 -> 부품 2 -> 합체 덧셈 -> 합체 힘 이벤트 -> 다음 문제 -> 10문제 완료 -> 합체 힘 재기 -> 로봇 매스몬 등급 결과
```
...
한 문제의 3단계 계산이 끝나면 화면 중앙에 합체 힘 이벤트가 뜹니다. 정답 문제는 힘 충전, 강화 부품, 부품 결함, 완성 신호, 합체 실패, 무지개 코어 중 하나가 나오며, 문제 안에서 한 번이라도 틀리면 부품 결함 처리만 적용됩니다. 완성 신호도 500점 보너스일 뿐 문제 수를 건너뛰지 않습니다. 무지개 코어는 800점 보상입니다. 보상은 합체 힘 하나로만 적용됩니다.
...
- Humanizer 학생 문구 QA: 보상 모달과 결과 화면에서 비율 표현을 빼고 `합체 힘`, `0점`, `무지개 힘이 800점 들어왔어요.`처럼 짧은 말로 고쳤습니다.
