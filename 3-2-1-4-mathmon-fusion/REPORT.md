...
- 결과 등급: 합체 힘 점수만 사용해 소형 0점, 중형 100점, 대형 200점, 거대 500점, 초거대 1500점, 전설 2000점 기준으로 공개
- 무지개 보상: 무지개 코어는 전설 바로 열기가 아니라 800점 보상으로 적용
...
합체 힘은 단일 중심 보상입니다. 보상 화면에서는 고정 점수 이미지를 크게 보여 주고, 한 판이 끝난 뒤 합체 힘 점수를 재어 정해진 기준에 맞춰 완성 로봇 등급을 공개합니다. 결과는 차시 자체 완결형 보상이며, 2차시의 행성 도달 구조와 같은 방식으로 `도달 등급`을 보여 줍니다.

## 3. 게임 흐름

```text
첫 화면 -> 설명 화면 -> 부품 1 -> 부품 2 -> 합체 덧셈 -> 합체 점수 이미지 -> 다음 문제 -> 10문제 완료 -> 합체 힘 재기 -> 로봇 매스몬 등급 결과
```
...
정답 문제는 여러 점수 이미지 중 하나가 랜덤으로 나오며, 문제 안에서 한 번이라도 틀리면 `-100점` 이미지만 나옵니다. `0점`은 점수 초기화가 아니라 변화 없음이고, `무지개`는 실제 `+800점` 보상입니다. `+500점`도 조기 종료가 아니라 큰 보상 이미지일 뿐이고, 어떤 보상이 떠도 10문제를 모두 푼 뒤에만 결과로 이동합니다.
...
백엔드 연동 지점은 `index.html`의 `LESSON_ID = "3-2-1-4-mathmon-fusion"`, `SCOREBOARD_API_URL`, `scoreboardBridge`, `scoreboardAnswers`, `scoreboardScreen`입니다. 업체는 정적 HTML을 열기 전에 `window.MATHMON_SCOREBOARD_API_URL`만 주입하면 됩니다. 4차시는 서버 점수로 `합체 힘`을 보내고, 문제마다 `partial1`, `partial2`, `fusion` 세 단계 선택과 합체 힘 보상(`normal`, `smallExplosion`, `megaFuel`, `instantLaunch`, `emptyTank`, `rainbowFuel`, `leak`)을 함께 보냅니다. `instantLaunch`는 기존 연동 호환을 위한 이벤트 id이며, 4차시에서는 조기 종료 없이 500점 보너스로만 처리됩니다. `emptyTank`는 0점 보상이고, `rainbowFuel`은 800점 보상입니다.

### 다시하기 결과 화면

`result-retry-generated.webp`는 호환용으로 남겨 둡니다. 현재 점수 기준에서는 0점도 소형 로봇 결과로 보여 줍니다.
...
- 이 목표 지도는 별도 점수판이 아니라 기존 중심 보상인 합체 힘을 시각화합니다. 로봇 외곽선에 맞추는 현재 표시, 하단 진행바, 원형 노드는 쓰지 않고 현재 단계 로봇이 켜진 목표판 이미지로 상태를 보여 줍니다.
...
- 상태별 목표 지도는 일부 자산에서 로봇 하체와 플랫폼이 190px 안에 온전히 들어오지 않아 runtime 적용에서 제외했습니다. 당시 runtime은 전체 플랫폼과 실루엣이 들어 있는 `play-robot-goal-strip-generated.webp` 한 장을 고정 목표판으로 사용하고, 현재 도달 단계는 빛 전환 효과와 접근성 상태만 바꿨습니다.
...
### 2026-07-09 상단 목표 지도 1232×166 상태 세트 교체

- 실제 문제 화면 상단바 안쪽 렌더 공간이 1280×800 Stage 기준 `1232×166`이어서, 기존 1280×190 상태 이미지 대신 `play-robot-goal-*-1232-generated.webp` 6장을 새로 연결했습니다.
- 새 세트 계약은 `6장`, `1232×166`, `각 장 로봇 슬롯 6개`, `현재 등급 1개만 컬러`, `나머지 5개 실루엣`, `잔상/중복/진행 노드 없음`, `로봇과 플랫폼 상하 잘림 없음`입니다.
- 생성 원본은 `play-robot-goal-six-1232-source.png`로 보관하고, 컨택시트는 `screenshots/play-robot-goal-1232-contact-sheet.png`입니다. 컨택시트에서 6장 모두 같은 배경·같은 플랫폼 위치·현재 1개 컬러 상태를 확인했습니다.
...
- 결과 기준을 합체 힘 점수만으로 다시 맞췄습니다. 소형은 0점, 중형은 100점, 대형은 200점, 거대는 500점, 초거대는 1500점, 전설은 2000점부터 공개됩니다.
- 무지개 코어는 전설 바로 열기가 아니라 800점 보상으로 처리합니다. 합체 실패는 0점 보상이며, 기존 점수를 0으로 되돌리지 않습니다.
- 보상 점수 이미지 8장을 다시 연결했습니다. 학생 화면에는 `+50점`, `+100점`, `+200점`, `+500점`, `-50점`, `-100점`, `0점`, `무지개` 이미지만 보이고, CSS 숫자 카드나 퍼센트 표기는 보이지 않습니다.
- Humanizer 학생 문구 QA: 보상 모달과 결과 화면에서 비율 표현을 빼고 `합체 점수`, `0점`, `무지개 점수 800점이 들어왔어요.`처럼 짧은 말로 고쳤습니다.
