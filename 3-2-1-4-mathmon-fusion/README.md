...
첫 화면과 결과 화면은 생성 이미지를 RasterStage 배경으로 씁니다. 첫 화면은 제목·목표·시작 버튼을 표준 오버레이로 두고, 결과 화면은 큰 `초거대 로봇!` 같은 등급 문구를 생성형 타이틀 이미지로 보여 줍니다. 단일 SVG 동적 레이어는 `9/10`, `100%` 같은 매 판 값과 `순위 보기`, `다시하기` 버튼 표면만 그립니다. 첫 화면은 `cover-robot-mathmon-generated.webp`를 사용해, 매스몬 자체가 둥글고 친근한 로봇형 캐릭터로 보이도록 했습니다. 생성 이미지에는 숫자를 넣지 않았습니다.
