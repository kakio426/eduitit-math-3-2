...
- 결과: 최종 합체 점수로 도달 로봇을 보여 줌. 소형 100점, 중형 300점, 대형 500점, 거대 1000점, 전설 1500점 기준
...
최신 보상 화면은 생성형 점수 이미지가 배경 위에 바로 떠 보이도록 구성했습니다. 학생이 보는 숫자가 실제 누적 점수입니다. `+50점`, `+100점`, `+200점`, `+500점`, `-50점`, `-100점`, `0점`, `무지개` 8개 점수 자산만 교체되며, 무지개는 실제 `+800점` 보상입니다.
...
- `play-robot-goal-ultra-generated.png/webp`: 이전 초거대 단계 보존 자산(현재 점수표 runtime에서는 사용하지 않음)
...
- `result-ultra-generated.png/webp`: 이전 초거대 단계 보존 자산(현재 점수표 runtime에서는 사용하지 않음)
...
첫 화면과 결과 화면은 생성 이미지를 RasterStage 배경으로 씁니다. 첫 화면은 제목·목표·시작 버튼을 표준 오버레이로 두고, 결과 화면은 큰 로봇 등급 문구와 `9/10` 같은 정답 수를 생성형 이미지로 보여 줍니다. 단일 SVG 동적 레이어는 실제 합체 점수, `순위 보기`, `다시하기` 버튼 표면만 그립니다. 첫 화면은 `cover-robot-mathmon-generated.webp`를 사용해, 매스몬 자체가 둥글고 친근한 로봇형 캐릭터로 보이도록 했습니다.

문제 화면은 생성 이미지 합체 공방 배경 위에 HTML/CSS 합체 보드를 올립니다. 상단 목표 지도는 3차시 지도처럼 생성 이미지가 로봇 실루엣과 합체 길을 담당합니다. 진행 상태는 하단바나 좌표 마커를 올리지 않고, 점수 기준에 맞는 목표 지도 이미지로 통째 교체합니다. 목표 지도는 `1280×190` 배너 슬롯으로 표시하고, 가로 배치는 그대로 유지합니다. 현재 기준은 소형 100점, 중형 300점, 대형 500점, 거대 1000점, 전설 1500점입니다. 보상 화면은 `+50점`, `+100점`, `+200점`, `+500점`, `-50점`, `-100점`, `0점`, `무지개` 생성형 점수 이미지 8장만 바꿔 보여 줍니다. 무지개는 실제 점수 `+800점`입니다. 이 점수 이미지는 투명 WebP라 보상 배경 위에 바로 떠 보이고, 네모 카드처럼 잘린 배경은 없습니다. 부품 1, 부품 2, 합체 수는 학생의 선택에 따라 채워지며, 마지막 단계에서 두 부품이 가운데로 합쳐지는 CSS 연출이 적용됩니다.
...
- `screenshots/08-result-retry.png`: 점수가 부족한 재도전 결과
