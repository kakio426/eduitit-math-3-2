...
- 방식: 피자 SVG를 보고 큰 분수 카드를 `분수 접시`에 놓는 1단계 조작형. 카드를 탭한 뒤 접시를 탭해도 제출됨
...
이번 문제 화면은 단순 선택 대신 **분수 카드를 접시에 놓는 행동**으로 바꿨습니다. 학생이 피자를 보고 `전체 중 색칠된 조각`을 하나의 분수 카드로 대응시키게 하기 위해서입니다. 일부 문제에서는 색칠 조각이 떨어져 있을 수 있어, 붙어 있는 모양보다 **전체 중 몇 조각인지 세는 것**이 중요하다는 점을 드러냅니다.
