...
- 차시 원본 해시: `3d5a4f1ec8996682e4154512ab5de00be9c0eb427362239532c4cc36b4078762`
...
- 선택지 보조 그림: 출판사 자료형 실행 WebP 8장과 문항 2·3·5·6의 생성 래스터 WebP 실행본 18장(원본 SVG는 소스 보관), 모두 실행 가로 512px 이상
- 흔적 획득 이미지: `assets/trace-earned-v1-generated.webp`, 512×512
...
문항 2·3·5·6의 모든 보기에도 같은 역할의 생성 래스터 WebP 장식 그림을 붙였다. 이
그림은 보기 뜻을 빠르게 읽게 할 뿐 역사·사회 사실의 근거가 아니다. 문항 2·6의 연결선은 학생이
...
- 흐름: `정답 확인 → 소셜몬 흔적 획득 모달 → (학생이 버튼을 누름) → 2·4문제 힌트 → 소셜몬 만나기`
- 흔적 획득 모달은 `소셜몬 흔적 획득!` 제목, 512×512 이미지, 누적 개수, 다음 버튼을
  함께 보여 주며 자동으로 사라지지 않는다. 2·4문제에서는 버튼을 누른 뒤에만 기존
  그림자·특징 힌트 모달로 이어진다.
...
- 6문항 대기 화면
- 대표 오답·정답 확인
- 오답/정답 뒤 소셜몬 흔적 획득 모달과 명시적 버튼 이동
...
- 인페이지 QA 훅 `auditVisualContracts`에 `matchBoardAudit`와
  `traceRewardModalAudit`를 연결해 이미지 형식·해상도·표시 크기·하단 여백·모달
  필수 요소를 함께 검사하도록 했다.
...
- 사용자 제보 viewport 1079×929의 Q2에서 선택지 6개가 모두 WebP로 로드되고 자연
  가로 512px, 표시 크기 73.37×56.10px이었다. `#questionWorkbench` 하단 빈 공간은
  89.10px으로 `matchBoardAudit` 상한 132px 안이다.
- 같은 viewport에서 정답 확인 후 `traceRewardModal`은 1.5초 뒤에도 열린 채였고,
  `소셜몬 흔적 획득!`·`흔적 2 / 6`·`힌트 보기`가 보였다. 버튼을 누른 뒤에만
  `traceRevealModal`이 열렸다.
