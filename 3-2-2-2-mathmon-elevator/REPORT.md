...
특히 2단계 보기에는 일의 자리만 골랐을 때의 숫자도 함께 넣어, 남은 십을 빠뜨리면 다른 답이 된다는 것을 계산판 안에서 직접 비교하게 했습니다.
...
문제 화면은 왼쪽에 작은 엘리베이터 장면, 오른쪽에 큰 문제와 세로셈 보드, 한 줄 지시문, 선택지만 둡니다. 층 목록, 올라갈 힘 설명, 힌트, 단계 탭은 기본 화면에서 접어 두어 학생이 지금 고를 것 하나만 보게 했습니다. 엘리베이터 차체는 CSS gradient/pseudo-element가 아니라 imagegen 원본에서 배경을 제거한 `elevator-car-generated.webp` 스프라이트입니다.

나눗셈 보드는 몫 두 자리, 나누는 수, 나누어지는 수를 같은 세로셈 기준선에 맞춰 보여 줍니다. 1단계에서는 몫 칸만 크게 보이고, 정답 뒤 2단계에서 남은 십과 내릴 수 칸이 나타납니다. 각 단계가 끝나면 해당 칸이 숫자로 채워지고, 마지막 단계에서는 최종 몫이 완성됩니다.
...
- Humanizer 학생 문구 QA: 문제 지시문을 `십의 자리 답을 골라요`, `내린 수를 골라요`, `일의 자리 몫을 골라요`처럼 한 행동으로 줄였습니다. 보상과 결과 문구도 변화량·도착 층·올라갈 힘처럼 학생이 화면에서 바로 볼 수 있는 말로 유지했습니다.
...
- 스크린샷 갱신: `screenshots/01-cover.png`, `screenshots/07-tablet-cover.png`

## 문제 화면 단일 행동 리디자인 (2026-07-07)

- 스킬 기준 보강: `.claude/skills/eduitit-mathmon-lesson/SKILL.md`의 문제 화면 계약을 `한 화면 한 행동` 기준으로 명확히 했습니다.
- 과밀 제거: 문제 화면 기본 노출을 큰 문제, 현재 세로셈 보드, 한 줄 지시문, 선택지로 줄였습니다. 힌트 버튼, 단계 칩, 층 목록, 올라갈 힘 설명은 기본 화면에서 보이지 않게 했습니다.
- 세로셈 정렬: 몫 칸, 나누어지는 수 칸, 내림 행을 같은 digit area 기준으로 맞췄습니다. 나누는 수는 왼쪽 열, 나누어지는 수는 오른쪽 세로셈 영역에 고정됩니다.
- 1단계 단순화: `남은 십 ?` 같은 문구 칸을 없애고, 1단계에서는 몫 칸만 보이게 했습니다. 남은 십은 정답 뒤 2단계에서 숫자 칸으로 나타납니다.
- 선택지 확대: 1단계 보기를 `몫 N | 남은 십 N` 두 칸이 한 카드 안에서 붙어 보이도록 바꿨습니다. 2단계 보기는 숫자만 보여 주어 지시문과 보기 문장이 싸우지 않게 했습니다.
- 문구 QA: 문제 지시문은 `십의 자리 답을 골라요.`, `내린 수를 골라요.`, `일의 자리 몫을 골라요.`처럼 한 문장에 행동 하나만 남겼습니다.

### 텍스트 넘침·요소 겹침 QA (2026-07-07)

- 확인 크기: 현재 인앱 브라우저 `918×897`(dpr 2), 데스크톱 `1280×800`, 태블릿 가로 `1024×768`
- 확인 상태: 문제 1단계, 문제 2단계, 최종 답 확인
- 결과: 문제 화면의 `overflowText`, Stage 밖 요소, 계산판-지시문-선택지 겹침 0건
- 상단 컨트롤 정렬: 현재 인앱 브라우저 `918×897`에서 단원 배지와 설정 버튼 `gap 16px`, `heightDiff 0`, `topDiff 0`, `overlapArea 0`으로 확인했습니다.
- 세로셈 숫자 정렬: 현재 인앱 브라우저 `918×897`에서 나누는 수와 나누어지는 수의 글자 크기 `33.6px`로 일치, 외곽 카드 높이 `70px`로 일치, 나누어지는 수 digit 카드/나누는 수 카드 높이 비율 `0.91`로 확인했습니다.
- 내림 단계 정렬: 현재 인앱 브라우저 `918×897`에서 `↓` 화살표 0건, 활성 계산 칸 1개(`#downNumberBox`), 아래 행 overflow 0건으로 확인했습니다.
- 내림 수 자리 표시: 3단계에서도 내린 수를 한 박스짜리 두 자리 수로 합쳐 쓰지 않고, 남은 십 칸과 일의 자리 칸을 따로 유지합니다. 현재 인앱 브라우저에서 `54 ÷ 2`의 3단계가 `1 | 4` 두 칸으로 보이고, `downNumberBoxLooksLikeFullTwoDigitNumber false`, `visibleDownBoxes 2`, `downBoxesOverlap false`, `overflow 0`으로 확인했습니다.
- 1단계 보기 배치: `십의 자리 답을 골라요.` 문구와 `몫 N | 남은 십 N` 카드 구조를 QA 스크립트에 고정했습니다. 데스크톱 QA에서 각 보기 카드의 두 칸이 버튼 높이와 반쪽 폭을 채우고, 텍스트 overflow 0건으로 확인했습니다.
- 스크린샷 갱신: `screenshots/03-problem.png`, `screenshots/03-problem-step2.png`, `screenshots/03-final-confirm.png`, `screenshots/10-tablet-problem.png`, `screenshots/11-tablet-final-confirm.png`
- 명령 검증: `node scripts/qa-lesson2-elevator.mjs`, `node scripts/check-stage-ratio.mjs` 통과

## 완성도 폴리시 패스 (2026-06-24)

- 선택지 단순화: 십의 자리 단계 선택지를 문장형에서 `몫 N` + `남은 십 N` 좌우 카드로 바꿔 한눈에 읽히게 했습니다.
