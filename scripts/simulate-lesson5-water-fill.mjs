...
const perfect = results.get('10'); assert(Object.keys(perfect).length >= 2, "perfect bucket should vary by luck"); assert(Object.keys(results.get('0')).length >= 1, "zero bucket should still produce a result"); const families = new Set(model.REWARD_EVENTS.map(event => event.family)); families.add(model.WRONG_REWARD_EVENT.family); assert(families.has('repair'), 'repair family missing'); console.log("LESSON5_WATER_FILL_SIMULATION: PASS"); console.log(JSON.stringify({ runs: options.runs, buckets: Object.fromEntries(results), rewardFamilies: [...families].sort() }, null, 2)); }
main();
