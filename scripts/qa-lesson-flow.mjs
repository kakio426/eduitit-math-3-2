...
async function readSnapshot(page) {
  return evaluate(page, `(() => {
    ...
    const visibleContentOutside = (node) => {
      ...
      const inside = {
        left: box.left + node.clientLeft * scaleX,
        top: box.top + node.clientTop * scaleY,
        right: box.left + (node.clientLeft + node.clientWidth) * scaleX,
        bottom: box.top + (node.clientTop + node.clientHeight) * scaleY
      };
...
async function auditConfiguredTypography(page, label) {
  ...
  if (config.instructionRequired === false || audit.circleChoiceLayout) {
...
async function auditResultPanelContainmentV2(page, label) {
  const audit = await evaluate(page, `(() => {
    ...
    if (!config) return null;
    if (!config.elements || typeof config.panelDetector === 'string') return { skipped:true };
    const stageNode = document.querySelector('.stage-shell');
    const scene = document.querySelector(config.sceneImage || '#resultBg');
    const tier = document.getElementById('screen-result')?.dataset.resultTier || '';
...
async function auditRuntimeBuildMetadata(page, lesson, label) {
  ...
  const source = fs.readFileSync(path.join(sourceDir, "lesson.json"), "utf8");
  const config = JSON.parse(source);
  const sourceFiles = config.sourceFiles || {};
  const modelSource = fs.readFileSync(path.resolve(sourceDir, sourceFiles.model || "model.js"), "utf8");
  const viewSource = fs.readFileSync(path.resolve(sourceDir, sourceFiles.view || "view.js"), "utf8");
  const sharedStyle = sourceFiles.style ? fs.readFileSync(path.resolve(sourceDir, sourceFiles.style), "utf8") : "";
  const localStylePath = path.join(sourceDir, "lesson.css");
  const localStyle = fs.existsSync(localStylePath) ? fs.readFileSync(localStylePath, "utf8") : "";
  const lessonCss = [sharedStyle, localStyle].filter(Boolean).join("\n");
  const expectedLessonJsonSha = createHash("sha256").update(source).digest("hex");
  const expectedBuildInputsSha = hashRuntimeBuildInputs({
    buildFingerprintHelperSource:fs.readFileSync(path.join(ROOT, "scripts", "runtime-build-fingerprint.mjs"), "utf8"),
    buildScriptSource:fs.readFileSync(path.join(ROOT, "scripts", "build-lesson.mjs"), "utf8"),
    engineCss:fs.readFileSync(path.join(ROOT, "_engine", "v1", "styles", "core.css"), "utf8"),
    engineRuntime:fs.readFileSync(path.join(ROOT, "_engine", "v1", "runtime", "core.js"), "utf8"),
    lessonCss,
    lessonJsonSource:source,
    modelSource,
    scoreboardCss:fs.readFileSync(path.join(ROOT, "_shared", "scoreboard", "scoreboard-ui.css"), "utf8"),
    scoreboardRuntime:fs.readFileSync(path.join(ROOT, "_shared", "scoreboard", "scoreboard-ui.js"), "utf8"),
    sharedAudioRuntime:fs.readFileSync(path.join(ROOT, "_shared", "audio", "mathmon-audio-v1.js"), "utf8"),
    template:fs.readFileSync(path.join(ROOT, "_engine", "v1", "template.html"), "utf8"),
    viewSource,
  });
  const runtime = await evaluate(page, `(() => { const node=document.querySelector('#mathmonRuntimeBuildMeta'); return node ? { lessonId:node.dataset.lessonId || '', commitSha:node.dataset.commitSha || '', lessonJsonSha:node.dataset.lessonJsonSha256 || '', buildInputsSha:node.dataset.buildInputsSha256 || '' } : null; })()`);
  assert(runtime?.lessonId === (config.id || ''), `${label}: runtime lesson id is stale`, { runtime, expectedLesson:config.id });
  const commitIsAncestor = /^[0-9a-f]{40}$/.test(runtime?.commitSha || "")
    && spawnSync("git", ["merge-base", "--is-ancestor", runtime.commitSha, "HEAD"], { cwd:ROOT }).status === 0;
  assert(commitIsAncestor, `${label}: runtime build commit is not reachable from HEAD`, { runtime });
  assert(runtime?.lessonJsonSha === expectedLessonJsonSha, `${label}: stale runtime lesson.json SHA`, { runtime, expectedLessonJsonSha });
  assert(runtime?.buildInputsSha === expectedBuildInputsSha, `${label}: stale runtime build inputs`, { runtime, expectedBuildInputsSha });
  return runtime;
}
  ...
  for (const item of cases) {
    const prepared = await evaluate(page, `(() => {
      ...
      window.__mathmonEngineQa.setState({
        ...
        completed:true, mistakeTouched:false, rewardPhase:'idle', rewardTransitioning:false,
        pendingRewardEvent:null, currentResult:null
      });
...
async function runViewport(page, lesson, pageUrl, viewport, seed) {
  ...
  const tutorialMode = await evaluate(page, "LESSON_CONFIG.tutorial?.mode || 'card-grid'");
  ...
  const firstTutorialLabel = tutorialMode === "poster-two-step" ? "다음" : "문제 시작";
  await waitUntil(page, `document.querySelector('.screen.is-active')?.id === 'screen-tutorial' && (document.getElementById('tutorialStartButton').textContent.trim() || document.getElementById('tutorialStartButton').getAttribute('aria-label')) === ${JSON.stringify(firstTutorialLabel)}`, `${viewport.name}: tutorial not shown`);
  shots.push(await screenshot(page, lesson, viewport, "03-tutorial-1"));
  await auditGeometry(page, `${viewport.name} tutorial`);
  if (tutorialNextButtonAsset) {
    await auditGeneratedActionButton(page, `${viewport.name} tutorial action button`, "#tutorialStartButton", tutorialNextButtonAsset, firstTutorialLabel);
  }
  ...
  if (tutorialMode === "poster-two-step") {
