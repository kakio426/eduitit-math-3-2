...
- 방식: 두 분수를 같은 길이 막대로 보여 주고, 더 큰 분수 카드를 `더 큰 분수` 승리 자리에 옮기는 1단계 조작형. 카드 탭만으로도 제출됨
...
이번 문제 화면은 더 긴 막대의 **분수 카드를 승리 자리로 옮기는 방식**입니다. 학생이 숫자 크기만 보고 고르기보다 막대 길이를 먼저 확인하고, 더 큰 분수를 물리적으로 골라내게 하려는 설계입니다.
