...
- 보상 룰렛(5종)·결과 보기·등급 트랙·정답 수 조건·오디오·Stage/설정 모달 계약·rAF 모션은 그대로 재사용하고 라벨만 줄다리기 테마로 바꿨습니다.
...
- 1180×740에서 첫 화면 `시작` 버튼 아트가 Stage 아래로 잘리는 문제를 보정했습니다. 짧은 높이 전용 media query로 제목/목표/버튼 스택을 조금 줄이고, 생성형 버튼 아트와 hitbox가 Stage 안에 남게 했습니다.
- 문제 화면 `더 큰 분수` 드롭존의 play 전용 압축 규칙을 보정해 Stage 환산 높이 90px 이상으로 유지되게 했습니다.
- 브라우저 QA: Playwright로 1280×800, 1024×768, 1180×740에서 첫 화면과 첫 문제, 정답 드래그 확인 상태를 다시 캡처했습니다. 1180×740 첫 화면 `#startButton`/버튼 아트 Stage 내부 정렬을 확인했습니다. 드래그 카드 최소 높이는 `129.0/154.6/140.2`, 드롭존 최소 높이는 `91.5/101.6/90.2` Stage px입니다.
- 텍스트 넘침·요소 겹침 QA: 위 세 화면 크기에서 첫 화면 제목·목표·시작 버튼, 문제 지시문, 분수 막대 카드, 드롭존, 하단 점수 스트립의 넘침·겹침 0건입니다. 콘솔 오류, pageerror, request failed, 400 이상 HTTP 응답 0건입니다.
- 추가 회귀 QA에서 태블릿/중간 높이 화면의 비교 막대 SVG 분수 라벨이 `getBBox()` 기준으로 SVG 박스 밖에 걸리는 신호가 있어, 문제 화면 막대 SVG를 `height:auto`로 보정했습니다. 4-4 단독 재검증은 1280×800, 1024×768, 1180×740의 33개 상태 캡처에서 실패 0건, overflow 0건, SVG 텍스트 영역 이탈 0건입니다.
