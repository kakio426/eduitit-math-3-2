...
- 보상: 한 문제 완료마다 상자 1칸이 채워지고 검사문 이벤트 1회가 적용됩니다. 정답 문제는 상자가 자람, 빠른 벨트, 고쳐서 보냄, 문이 열림, 잠깐 멈춤, 무지개 빛 중 하나가 나오고, 오답이 있었던 문제는 고쳐서 보내는 상자로 표시합니다.
- 상자 힘: 랜덤 이벤트로 오르내리는 값입니다. 결과 화면에서는 장소가 아니라 상자 단계 막대와 함께 보여 줍니다.
- 정답 잠금: 상자 힘이 높아도 정답 수가 모자라면 높은 상자가 열리지 않습니다. 은빛은 6문제, 금빛은 7문제, 왕관은 9문제, 무지개는 10문제가 필요합니다.
- 최종 보상: 오늘 만든 상자 등급을 크게 공개합니다. 냥냥몬은 큰 상자를 축하하는 조연으로만 둡니다.

## 3. 게임 흐름

```text
첫 화면 -> 설명 화면 -> 0 뗀 곱 선택 -> 0 개수 선택 -> 답 완성 -> 상자 검사문 통과 -> 10문제 완료 -> 상자 등급 공개
```
...
한 문제의 두 단계가 끝나면 상자가 컨베이어 위를 지나 검사문 앞에서 잠깐 멈춥니다. 잠깐 기다린 뒤 `종이 상자`, `노란 상자`, `튼튼 상자`처럼 현재 상자 등급이 짧게 나옵니다. 빠른 벨트, 문이 열림, 잠깐 멈춤, 무지개 빛은 상자 힘을 바꾸는 이벤트로만 작동하고, 결과 화면으로 일찍 넘어가지 않습니다. 문제 안에서 한 번이라도 틀리면 `고쳐서 보냄`이 적용됩니다.

보상 모달은 설명 문장을 없애고 현재 등급 상자 이미지와 한 줄 라벨만 보여 줍니다. 버튼은 다음 행동만 말하도록 `다음` 또는 `보기`로 둡니다.

### 결과 화면

결과 화면은 `result-class-generated.webp` 공통 배경 위에 오늘 만든 상자를 크게 얹습니다. 장소 배경과 도착 표현은 쓰지 않습니다. 상자 옆에는 냥냥몬이 축하하는 조연으로만 나오고, 오른쪽에는 상자 이름, 맞힌 문제, 다음 상자 조건, 상자 힘 막대만 짧게 보여 줍니다. 상자 등급은 종이·노란·튼튼·은빛·금빛·왕관·무지개 7단계입니다. 높은 상자는 상자 힘과 정답 수가 함께 필요하며, 무지개 상자는 10문제를 모두 맞히고 무지개 빛이 나와야 열립니다.

## 5. 매스몬 역할

3차시의 보상 주인공은 매스몬이 아니라 오늘 만든 상자입니다. 실행 화면에서는 냥냥몬을 고정 동행으로 쓰고, 결과 화면에서는 큰 상자를 축하하는 역할로만 둡니다. 정답 수나 상자 등급에 따라 다른 매스몬이 나오지 않습니다.
