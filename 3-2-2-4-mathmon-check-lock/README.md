...
- 문제: 맞는 나눗셈 카드와 틀린 나눗셈 카드를 섞어 10개 랜덤 출제
- 방식: `묶음 만들기 -> 남은 것 붙이기 -> 처음 수 보기` 3단계 선택형. 틀린 카드는 `틀린 곳 찾기`가 한 번 더 나옴
...
예를 들어 `47 ÷ 5 = 9 ... 2`는 `5개씩 9묶음 = 45`, `45개 + 남은 2개 = 47` 순서로 처음 수를 다시 만들어 봅니다. 가끔 일부러 틀린 나눗셈 카드도 나오므로, 학생은 마지막에 다시 만든 수가 처음 수와 같은지 보고, 다르면 `몫`, `남은 것`, `처음 수` 중 어디가 달랐는지 고릅니다.
...
- `screenshots/03b-problem-step1-filled.png`: 묶음 막대 채움
- `screenshots/03c-problem-step2-filled.png`: 남은 것 붙임
- `screenshots/03d-mismatch-locate.png`: 틀린 곳 찾기
- `screenshots/03e-mismatch-confirm.png`: 틀린 카드 확인
