...
문제 화면은 3-2-2-3의 실제 비율을 기준으로 다시 만들었습니다. 왼쪽 `23.4375%×82%`에는 킹드래곤몬과 현재 금고가 이어지는 세로 장면 하나를 두고, `지금 / 열쇠 빛 / 다음`은 장면 위 정보층으로만 표시합니다. 오른쪽 약 70%에는 큰 문제, 검산 자물쇠, 한 줄 안내, 선택지만 한 열로 둡니다. 검산 SVG의 사각 표면은 하나뿐이며 몫·나머지는 비교가 필요한 단계에서만 원형 기능 슬롯으로 보입니다.
...
- 열린 보상은 사건 이미지, 열쇠 빛 변화량, `다음` 버튼만 보여 줍니다.
- `mathmon-unified-reward-v1`을 적용해 열쇠 빛은 `0~100`으로 계산합니다.
- 정답 사건은 `64%/+6~10`, `15%/-5~-2`, `12%/+14~22`, `5%/+30`, `3.8%/0`, `0.2%/100`이며 오답은 문제당 최초 1회 `-6~-3`입니다.
- `0` 사건은 지금까지 모은 열쇠 빛을 지우지 않고 그대로 둡니다. 희귀 무지개 열쇠는 최고 결과를 엽니다.
- 일반 결과 기준은 `(열쇠 빛/최소 정답 수) 0/0, 15/2, 35/4, 55/6, 78/8`입니다.
...
- 왼쪽 보상판은 Stage 폭 18%, 높이 55% 이상이어야 합니다. 현재 그림·이름·열쇠 빛·다음 금고를 모두 담고 계산 작업 영역과 8px 이상 떨어져야 합니다. 작은 숫자 배지로 줄어들거나 어느 하나가 빠지면 실패합니다.
...
node scripts/qa-lesson2-check-lock.mjs 20260728
