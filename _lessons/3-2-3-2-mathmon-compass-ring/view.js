...
function compassVisualLabel(radius, opening, visualState) {
  if (visualState === "waiting") return `중심에서 원까지의 길이가 ${radius} cm인 원`;
...
function targetRadiusMarkup(radius) {
  ...
  return `
    <circle class="target-circle" cx="${centerX}" cy="${centerY}" r="${COMPASS_TARGET_RADIUS}"/>
...
function onRewardPrepare({ beforePower, beforeResult }) {
  ...
  if (ui.rewardScene instanceof HTMLImageElement) ui.rewardScene.src = LESSON_CONFIG.imageAssets.rewardClosed;
}
...
function onRewardReveal({ event, beforePower, afterPower, beforeResult, afterResult }) {
  ...
  if (ui.rewardScene instanceof HTMLImageElement) ui.rewardScene.src = eventImage;
  const reducedMotion = matchMedia("(prefers-reduced-motion: reduce)").matches;
