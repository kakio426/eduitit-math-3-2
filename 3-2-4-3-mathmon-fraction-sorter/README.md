...
- 방식: 중앙의 분수 카드를 `진분수 통`/`가분수 통`/`대분수 통`에 넣는 1단계 조작형. 카드를 탭한 뒤 통을 탭해도 제출됨
...
이번 문제 화면은 분수와 원 그림을 하나의 **큰 분수 카드**로 묶고, 아래 통에 넣게 했습니다. 분류라는 수학 행동이 화면 조작과 바로 맞물리도록 해, 학생이 “이 분수는 어떤 종류인가?”를 누르기보다 실제로 나누어 담는 느낌으로 판단하게 했습니다.
...
- 공용 매스몬 관리는 `_shared/mathmon/fraction-pack/`와 `_shared/mathmon/catalog.json`에서 합니다. 이 차시는 `mathmon-fr-03-sorterbeaver` 콘셉트를 기준으로 한 생성형 커버/결과 장면을 사용하며, 별도 HTML 매스몬 오버레이는 쓰지 않습니다.
