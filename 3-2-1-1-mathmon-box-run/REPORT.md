...
첫 화면은 V2 매스몬 자산과 상자런 대표 장면을 합성한 커버를 사용합니다. 게임 제목, 매스몬, 상자, 배움 주제, 시작 버튼을 한 장면 안에 넣어 학생이 들어오자마자 `어떤 게임인지` 바로 느끼게 했습니다. 실제 조작은 이미지 위의 `시작` 버튼 위치에 투명 HTML 버튼을 얹어 처리하므로, 화면은 그림처럼 보이지만 클릭과 접근성은 유지됩니다.
...
| --- | --- | ---: | ---: | --- |
| 1 | 몽글몬 | 0점 이상 | 0개 이상 | 몽글몽글 힘을 모아 첫 상자를 열어 주는 매스몬입니다. |
| 2 | 콩콩몬 | 500점 이상 | 2개 이상 | 정답을 찾으면 콩콩 뛰며 기뻐하는 매스몬입니다. |
| 3 | 젤리몬 | 5,000점 이상 | 4개 이상 | 말랑한 계산 감각으로 상자를 척척 고르는 매스몬입니다. |
| 4 | 반짝몬 | 50,000점 이상 | 5개 이상 | 풀이가 맞을 때마다 반짝 빛나는 매스몬입니다. |
| 5 | 데굴몬 | 500,000점 이상 | 6개 이상 | 큰 점수도 데굴데굴 굴려 모으는 매스몬입니다. |
| 6 | 루미몬 | 5,000,000점 이상 | 7개 이상 | 문제 길을 환하게 밝혀 주는 매스몬입니다. |
| 7 | 큐브몬 | 50,000,000점 이상 | 8개 이상 | 계산 블록을 차곡차곡 쌓는 매스몬입니다. |
| 8 | 스파크몬 | 500,000,000점 이상 | 8개 이상 | 높은 점수에 번쩍 힘을 붙이는 매스몬입니다. |
| 9 | 오로라몬 | 5,000,000,000점 이상 | 9개 이상 | 행운빛을 펼쳐 특별한 상자를 부르는 매스몬입니다. |
| 10 | 크라운몬 | 50,000,000,000점 이상 | 10개 | 이번 판의 최고 상자런을 장식하는 매스몬입니다. |
...
매스몬 원본 PNG와 생성 원본은 `_shared/mathmon/core-pack-v2/`에서 관리하고, 1차시 폴더에는 실행에 필요한 WebP만 둡니다.
