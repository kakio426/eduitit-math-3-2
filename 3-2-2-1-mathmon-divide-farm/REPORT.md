...
- 학생은 십의 자리 몫, 일의 자리 몫, 전체 몫을 각각 직접 결정합니다.
- 십의 자리 단계는 묶음 수 `2`가 아니라 실제 값 `20`을 입력·확인합니다.
- 오답은 학생이 쓴 양을 모든 바구니에 보여 주고, 전체보다 적으면 `남아요`, 많으면 `부족해요`로 비교합니다.
- 정답 뒤에만 생성형 채소가 학생의 답대로 바구니에 놓입니다.
- 마지막에는 완성식이 보이고 학생이 `수확 보기`를 눌러 보상 화면으로 이동합니다.
- 정답 경로의 수학적 판단은 문제당 3번, 물리 입력은 모든 문제에서 8회입니다.
...
현재 `index.html`이 참조하는 WebP의 `naturalWidth/naturalHeight`, 실제 렌더, 투명 배경, 상태별 파일명을 브라우저에서 확인했습니다. 결과는 모두 해당 `result.image`를 실제 배경으로 사용합니다.
...
문제 화면 설명은 한 문장에 행동 하나만 두고, 보상 화면의 보이는 문구도 한 번에 한 줄로 제한했습니다. `점수`, `게이트`, `이벤트`, `보상 구조` 같은 제작자 말을 학생 화면에 쓰지 않았습니다.

## 텍스트 넘침·요소 겹침 QA

- 화면 크기: `1280×800`, `1024×768`, `918×897`.
- 확인 상태: 표지, 설정, 설명 1·2, 문제 대기, 적음/많음 오답, 단계별 정답 확인, 마지막 완성식, 보상 닫힘·열림, 증가·감소·0·정답 수 제한·황금밭, 결과 6단계, 전국 순위.
- 결과: 텍스트 넘침 0건, 의도하지 않은 요소 겹침 0건, Stage 밖 조작부 0건, 깨진 이미지 0건.
...
현재 캡처는 이번 코드에서 다시 만든 `screenshots/engine-flow-*.png`입니다.
