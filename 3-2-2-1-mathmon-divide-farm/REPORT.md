...
```text
첫 화면 -> 설명 1 -> 설명 2 -> 십 묶음 나누기 -> 낱개 나누기 -> 몫 확인 -> 수확 보기 -> 보상 -> 결과
```
...
- 문제 화면 기본 노출을 큰 문제, 대표 바구니, 한 줄 지시, 선택지로 고정했습니다.
- 문제 풀이 중에는 왼쪽 농장 장면은 유지하되 상태 글자·미터를 숨기고, 오른쪽 수확물 식·단계칩·힌트를 숨겨 한 화면에 한 가지 행동만 남겼습니다.
- 바구니 여러 개에 같은 값을 반복해서 쓰지 않습니다. 큰 대표 바구니 1개에 `?`, `10개`, `10 + ?`, `10 + 1`, `11개`처럼 지금 필요한 값만 보이고, 작은 바구니 표식은 나누는 수만 보여 줍니다.
- 선택지는 `10개씩`, `1개씩`, `남지 않아요`, `담지 않아요`처럼 바로 읽히는 말로 바꿨습니다.
...
- 마지막 정답 뒤에는 선택지를 숨기고 대표 바구니에 `41개` 같은 완성값을 남깁니다.
...
- 문제 풀이 중 왼쪽 농장 장면 유지, 대표 바구니 1개만 노출, 상태 글자·미터·힌트·수확물 식·단계칩 숨김 확인
...
- 문제 화면 QA: 기본 화면에 큰 문제, 대표 바구니, 한 줄 지시, 선택지만 보임
...
- `흉작`은 `빈 바구니`로 바꿈
- `대풍`은 `큰 풍년`으로 바꿈
- `십 묶음을 먼저 나눠요.`, `남은 낱개를 나눠요.`, `바구니에 담아 볼까요?`처럼 한 문장에 행동 하나만 남김
- 자리값을 붙인 어색한 숫자 표현을 쓰지 않고 `10개씩`, `남지 않아요`처럼 바로 읽히는 말로 바꿈
