...
- 묶음 숫자표 회귀: 당근 아래 `10` 표를 묶음 수에 따라 4단계로 조절합니다. 표 높이는 `22~26px`, 글자 크기는 `15.2~17.92px`이며, 모든 문제 화면에서 `scrollHeight <= clientHeight`, 원본 카드 밖 이탈 `0px`, 묶음 사이 겹침 `0px`을 자동 검사합니다.
