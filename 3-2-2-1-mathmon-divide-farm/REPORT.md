...
- 나누는 중에는 특정 칸만 노랗게 강조하지 않습니다. 모든 칸을 같은 칸으로 보이게 하고, 확인 뒤에만 첫 칸을 `한 칸` 대표값으로 보여 줍니다.
- 4지선다 선택지는 제거했습니다. 학생은 당근을 고른 뒤 넣을 칸을 직접 누르거나, 당근을 칸으로 드래그합니다. 당근만 누르면 자동으로 들어가지 않습니다.
...
- 당근만 누르면 배치되지 않고, 당근 선택 뒤 칸을 눌러야 들어가는지 확인
...
- Codex 브라우저에서 당근 탭은 선택만 되는지, 칸 탭과 드래그로만 배치되는지 각각 확인
...
- `흉작`은 `빈 바구니`로 바꿈
- `대풍`은 `큰 풍년`으로 바꿈
- `10개 묶음을 골라 칸에 넣어요.`, `낱개 당근을 골라 칸에 넣어요.`, `왼쪽 바구니에 담아요.`처럼 한 문장에 행동 하나만 남김
