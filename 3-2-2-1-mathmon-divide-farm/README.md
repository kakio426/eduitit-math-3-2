...
큰 숫자 패드와 답 선택 버튼을 쓰지 않습니다. 한 화면에는 현재 나눠 담을 당근과 바구니만 크게 보여 줍니다.
...
4. 다음 화면에서 `8 ÷ 4`와 `한 바구니에 몇 개씩?`을 보고 낱개 당근도 모두 끌어 옮깁니다.
5. 마지막에는 `20 + 2 = 22`, `42 ÷ 2 = 21`, `한 바구니에 21개`가 한 화면에 완성됩니다. 몫을 다시 입력하지 않습니다.

문제당 수학적 판단은 `묶음을 똑같이 나누기`, `낱개를 똑같이 나누기` 2번입니다. 학생 요청에 따라 모든 당근을 직접 옮기며, 문제당 정답 경로 입력 수는 최소 4회, 중앙값 12회, 평균 10.87회, 최대 18회입니다. 문제 은행은 유효한 식 30개에서 매 판 10개를 랜덤으로 뽑고, 한 판에서 같은 식을 반복하지 않습니다.
...
흐름 QA는 `1280×800`, `1024×768`, `918×897`에서 설명 1·2, 문제 대기, 대표 오답, 묶음 확인, 낱개 대기·확인, 최종 완성식, 보상 닫힘·열림, 결과를 실제 렌더와 rect로 확인합니다. 문제 단계마다 선택지 3개, 터치 영역 42px 이상, 형제 영역 겹침 0px, Stage 밖 넘침 0px을 검사합니다.
