...
```json
{
  ...
  "panelDetector": {
    ...
    "searchRectByTier": {},
    "expectedDetectedRect": null,
    "expectedDetectedRectTolerancePx": 1,
    "forbiddenVisibleSelectors": [".result-panel-art"],
    "threshold": {},
    "minRunWidth": 180
  },
  "elements": {
...
3. 공통 빈 판을 쓰는 결과 세트는 `expectedDetectedRect`로 픽셀 판의 기준 좌표를 고정한다. 모든 단계가 이 rect를 `1px` 이내로 맞춰야 하며, 단계마다 판을 다시 생성해 위치가 흔들리거나 기존 판을 한 겹 더 얹으면 실패한다.
4. `forbiddenVisibleSelectors`에 등록한 레거시 패널/카드 레이어가 보이지 않는지 검사한다. 배경에 이미 빈 판이 있는데 `.result-panel-art` 같은 두 번째 판을 올려 시각이 겹치는 구현은 금지한다.
5. 검출 판에서 안전 영역을 `24px`씩 줄여 `panelSafeRect`를 만든다.
6. 제목·진행값·막대·정답 수·다음 목표·다시하기 아트·다시하기 hitbox의 실제 보이는 rect를 측정한다. 모든 rect가 안전 영역 안에 있고 형제 교차가 `0px`인지 확인한다.
7. 동적 값 묶음의 공통 중심축 오차는 `1px`, 버튼 아트와 hitbox 네 변 오차는 `1px` 이하로 확인한다. 숨긴 다음 목표는 `display:none`과 `0×0`이어야 한다.
8. 모든 결과 단계와 등록 viewport를 반복한다. 단계마다 판 크기와 안전 영역이 달라지면 실패한다.
9. 실행 메타데이터의 커밋 SHA와 `lesson.json` SHA-256이 하네스가 검사하는 현재 파일과 같은지 확인한다. 화면에는 보이지 않는 `aria-hidden` 디버그 요소로만 둔다.
10. 실제 검출 결과판과 단계별 주 보상물 경계로 위 비율을 계산하고, 금지 selector·문구와 보이는 정보 노드 수를 검사한다.

## 반복되는 결과 화면 실패의 예방

결과 장면 세트를 교체할 때는 모든 단계의 원본을 같은 `blank-panel` 레이아웃에서 파생한다. 배경 이미지에 제목·정답 수·버튼을 구워 넣은 원본과, HTML/CSS로 다시 만든 패널을 섞지 않는다. 한 장만 판이 좁거나 좌우로 밀리면 대표 캡처가 예뻐도 세트 전체를 다시 맞춘다. 자산 컨택시트와 브라우저 결과 컨택시트에서 `expectedDetectedRect`, 독립 제목/정답 수/다시하기 아트, 보상물 우세를 6단계 전부 확인한다.
