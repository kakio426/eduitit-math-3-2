...
12. **한 화면 한 행동 3초 검사**: 문제 화면 스크린샷에서 학생이 볼 기본 요소가 큰 문제·현재 단계·한 줄 지시·선택지뿐인지 확인한다. 풀이판 해석, 탭 선택, 힌트 읽기, 보상 확인, 선택지 고르기를 한 화면에서 동시에 하게 만들면 즉시 줄인다.
13. **Stage 비율 검사**: 모든 `index.html`이 `16:10`/`1280×800` 계약을 지키는지 루트에서 `node scripts/check-stage-ratio.mjs` 실행.
14. **상태 이미지 세트 QA**: 한 슬롯에서 바뀌는 생성 이미지 세트가 있으면 개수 계약, 치수, 컨택시트, 브라우저 렌더 크기, 데스크톱/태블릿 캡처를 확인한다. 한 장이라도 필수 슬롯 수가 빠지거나 잘리면 세트 전체 실패다.
15. **검증·배포**: `references/verification.md` 통과 → 배포는 `eduitit-main-release` + GitHub Pages, 공개 URL `curl -I -L` 200 확인.
